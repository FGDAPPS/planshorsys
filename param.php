...
function sousCondition(that) {
 if (that.value == "oui") {
     ...
     if (that.value == "non") {
       document.getElementById("ChoixOffre").style.display = "block";
    } else {
        document.getElementById("ChoixOffre").style.display = "none";
    }
...
<select name="type" onchange="artOrGrp(this);">
 ...
 <label>Articles :</label>  <select name="article[]" onchange="articleSetting(this)" multiple="">
    <option  value=""></option> 
    <option value="Dalaa 60U">Dalaa 60U</option>
    <option value="Dalaa 120U">Dalaa 120U</option>
...
<label>Par :</label>
<select name="unitOrCaisseSousCondition" onchange="SousCondUnOrcCheck(this);">
    <option value=""></option>
...
<label>Entrez le chiffre d'affaire:</label>
<input name="chiffreAffaireSousCond" type="number"  onchange="valeurSet(this);">
</div>
...
<input name="nbrUCSousCond" type="number" onchange="valeurSet(this);">
...
<div id="ChoixOffre" style="display: none;">
 <label> Offre </label>
...
if($offre=='REMISE'){
echo 'Taux de remise: '.$taux.'%';
}elseif($offre=='GIFT'){

  echo 'Article gift: '."<br>";
